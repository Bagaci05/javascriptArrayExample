...
for(let i =0; i<numberOfItems;i++){ //Ismétlőciklus létrehozása
    cars.push( //listához hozzáadjuk az elemet amit be fogunk kérni 
            prompt( //Bekérjük az elemet
                parseInt(i+1)+". Autó neve")); 

    cost.push( //listához hozzáadjuk az elemet amit be fogunk kérni 
            prompt( //Bekérjük az elemet
                parseInt(i+1)+". Autó ára"));
}

let finaltext = "Az autóink és áraik: \n"; //Létrehozunk egy végső szöveget anit ki fogunk írni
for(let i = 0; i<cars.length;i++){ //Ismétlő cikls ami végigfuta listánk elemein (Meg lehet oldani forin-el is)
